...
@ExportAsDevService
@ConfluenceComponent
public class ConfluenceAddOnUserService implements ConnectAddOnUserService
{
    ...
    private static final Logger log = LoggerFactory.getLogger(ConfluenceAddOnUserService.class);
    private final CrowdClientProvider crowdClientProvider;
    private final HostProperties hostProperties;

    @Autowired
    public ConfluenceAddOnUserService(ConnectAddOnUserProvisioningService connectAddOnUserProvisioningService,
            ConnectAddOnUserGroupProvisioningService connectAddOnUserGroupProvisioningService, CrowdClientProvider crowdClientProvider,
    ...
    public void disableAddonUser(String addOnKey) throws ConnectAddOnUserDisableException
    {
        ...
        try
        {
            ...
            if (null != user)
            {
                ...
                try
                {
                    crowdClientProvider.getCrowdClient().updateUser(userTemplate);
                }
                catch (InvalidAuthenticationException | InvalidUserException | OperationFailedException | ApplicationPermissionException | UserNotFoundException e)
                {
            ...
            throws OperationFailedException, InvalidCredentialException, ApplicationPermissionException, ApplicationNotFoundException, ConnectAddOnUserInitException, UserNotFoundException, InvalidAuthenticationException
    {
        ...
        try
        {
            ...
            userTemplate.setDisplayName(addOnDisplayName);
            crowdClientProvider.getCrowdClient().addUser(userTemplate, PasswordCredential.NONE);
            user = crowdClientProvider.getCrowdClient().getUser(userTemplate.getName());
    ...
    private User findUserWithFastFailure(String username, Exception userAlreadyExistsException) throws ApplicationNotFoundException
    {
        ...
        if (null == user)
        {
            ...
            throw new RuntimeException(String.format("The %s %s said that the %s '%s' did not exist, then that it could not be created because it does exist, then that it does not exist. Find a Crowd coder and beat them over the head with this message.",
                    CrowdClient.class.getSimpleName(), crowdClientProvider.getCrowdClient(), User.class.getSimpleName(), username), userAlreadyExistsException);
        }
    ...
    private User findUserByUsername(String username) throws ApplicationNotFoundException
    {
        ...
        try
        {
            user = crowdClientProvider.getCrowdClient().getUser(username);
        }
