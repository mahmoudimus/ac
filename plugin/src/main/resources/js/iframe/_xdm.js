(this.AP || this._AP).define("_xdm", ["_events", "_base64"], function (events, base64) {
  ...
  function XdmRpc($, config, bindings) {
    ...
    //  - xdm_p contains the easyXDM protocol mode, and is always set to 1 to indicate postMessage; this exists
    //    (only until AC-451 is addressed) for compatibility with the OAuth1a url signatures generated by the
    //    server, as this XdmRpc implementation only uses postMessage to communicate with the host window
    if (!/xdm_e/.test(loc)) {
