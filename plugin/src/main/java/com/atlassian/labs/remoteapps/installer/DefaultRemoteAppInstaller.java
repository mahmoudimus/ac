...
public class DefaultRemoteAppInstaller implements RemoteAppInstaller
{
    ...
    {
        /*!#start
        The installation process begins by constructing a GET to the Remote App to retrieve its registration
        ...
        <ul>
          ...
          <li><strong>publicKey</strong> - The OAuth public key of the host application, used to sign all future
            requests to the Remote App such as web hook POSTs, and Confluence macro retrieval queries.</li>
          <li><strong>baseUrl</strong> - The base URL of the host application, excluding the final slash.</li>
          <li><strong>description</strong> - The description of the host application for display purposes</li>
        </ul>
         */
        ...
        try
        {
            return (String) request.executeAndReturn(
                new ReturningResponseHandler<Response, String>()
                {
                    ...
                    {
                        /*!
                       If the request for the descriptor XML doesn't return with an HTTP 200 status code, an exception is thrown, resulting
                        ...
                        */
                        ...
                        /*!
                       The successfully returned XML descriptor is validated against the XML schema.  If the incoming
                         document uses the XML schema namespaces, great, but if not, the namespace is applied to the
                         document and the validation is then ran.  The XML schema can be found at:
                         <pre>
                           https://HOST_APPLICATION_BASE_URL/rest/remoteapps/latest/installer/schema/remote-app
                         </pre>
                        ...
                        */
                        ...
                        /*!
                       The app key, as defined in the registration XML document, must be valid for the installation user.
                       This means, the app key must either not be used by an existing app, or if it is, the installation
                       user must have the appropriate permissions to upgrade or modify that app.  If either of these
                        ...
                        */
                        ...
                        /*!
                       With the app key validated, the previous app with that key, if any, are uninstalled.
                        */
                        ...
                        /*!
                       The registration XML is then processed through second-level validations to ensure the values
                       are valid for this host application.  Among these include,
                       <ul>
                         <li>The display-url property is checked to ensure it shares the same stem as the registration URL </li>
                         <li>If any global modules such as Confluence macros are used, the installation user should have the correct permission to install global apps as the Remote App will then be treated as a global app.</li>
                         <li>
                         <li>If any permissions are specified, the installation user must be an administrator</li>
                       </ul>
                        */
                        ...
                        try
                        {
                            moduleGeneratorManager.getApplicationTypeModuleGenerator().validate(
                                    root, registrationUrl, username);
                            ...
                            if (moduleValidator.getUpgradedAccessLevel() != null)
                            {
                                accessLevel = moduleValidator.getUpgradedAccessLevel();
                            }
                        ...
                        root.addAttribute("access-level", accessLevel);

                        /*!
                        Finally, the descriptor XML is transformed into an Atlassian OSGi plugin artifact and installed
                       into the host application.  Part of this process involves extracting any Internationalization keys
                       out of the registration XML to be inserted into the plugin artifact as the expected properties file.
                        */
                        ...
                        /*!
                        The registration process should only return once the Remote App has successfully installed and
                           started.
                        */
        ...
        /*!
        Any exceptions thrown in the installation process will result in a error message returned to the user
         */
        ...
        {
            log.warn(
                    "Unable to retrieve registration XML from '{}' for user '{}' due to: {}",
    ...
    private class ValidateModuleHandler implements ModuleGeneratorManager.ModuleHandler
    {
        ...
        private final String pluginKey;
        private String upgradedAccessLevel;
            ...
            if (generator.getClass().getAnnotation(GlobalModule.class) != null)
            {
                if (!permissionManager.canInstallGlobalRemoteApps(username))
                {
                    throw new PluginParseException(
                            "Cannot install global remote apps by user " +
                                    "'" + username + "'");
                }
                upgradedAccessLevel = "global";
            }
            props.putAll(generator.getI18nMessages(pluginKey, element));
        }

        public String getUpgradedAccessLevel()
        {
            return upgradedAccessLevel;
        }
    }
}
