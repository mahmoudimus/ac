...
public class StashConnectUserService implements ConnectUserService
{
    ...
    @Autowired
    public StashConnectUserService(UserAdminService userAdminService, UserService userService)
    {
    ...
    public UserKey getUserKeyForAddon(@Nonnull String addonKey, @Nonnull String addonDisplayName) throws ConnectAddOnUserInitException {
        return toUserKey(getOrCreateUser(addonKey, addonDisplayName));
    }

    @Override
    public boolean isUserActive(@Nonnull UserKey userKey) {
        StashUser user = getUserByKey(userKey);
        return user != null && user.isActive();
    }

    @Override
    public void setUserActiveForAddon(@Nonnull String addonKey, final boolean active)
    {
    ...
    private StashUser getOrCreateUser(String addOnKey, String displayName)
    {
        ...
        StashUser user = userService.getServiceUserByName(username, true);
        if (user == null)
        {
