 ...
 *
 *
 * To receive webhook events, you add-on needs to include the webhook module declaration in its JSON descriptor. The
 * declaration indicates the URL of the local resource at which it will receive the notification. In other words, the
 * Atlassian application will send an HTTP POST to this resource in response to an application event. The add-on code
 * that handles the POST should process any information passed in the body of the message, as appropriate. Each webhook
 * POST sent to the add-on will also include the JWT authentication headers that allow the add-on to validate the
 * authenticity of that request.
 *
