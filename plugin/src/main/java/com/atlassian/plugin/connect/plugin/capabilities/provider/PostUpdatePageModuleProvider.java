...
@Component
public class PostUpdatePageModuleProvider extends AbstractGeneralPageModuleProvider
{
    @Autowired
    public PostUpdatePageModuleProvider(IFrameRenderStrategyBuilderFactory iFrameRenderStrategyBuilderFactory,
    ...
    public List<ModuleDescriptor> provideModules(ConnectModuleProviderContext moduleProviderContext, Plugin theConnectPlugin, String jsonFieldName, List<ConnectPageModuleBean> beans)
    {
        ...
        {
            ConnectPageModuleBean postUpdateBean = beans.get(0);

            ModuleDescriptor descriptor = new PostInstallPageModuleDescriptor();
            descriptor.init(theConnectPlugin, new DOMElement("connectPostUpdatePage").addAttribute("key",
                    postUpdateBean.getKey(moduleProviderContext.getConnectAddonBean())));
