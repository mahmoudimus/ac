...
public class CachingHttpAsyncClient implements HttpAsyncClient {
    ...
    @Override
	public Future<HttpResponse> execute(HttpHost target, HttpRequest request, FutureCallback<HttpResponse> futureCallback) {
        HttpContext defaultContext = null;
        return execute(target, request, defaultContext, futureCallback);
    }

    
	@Override
	public <T> Future<T> execute(HttpAsyncRequestProducer httpAsyncRequestProducer,
			HttpAsyncResponseConsumer<T> httpAsyncResponseConsumer, FutureCallback<T> futureCallback) {
		return execute(httpAsyncRequestProducer, httpAsyncResponseConsumer, null, futureCallback);
	}

	@Override
	public <T> Future<T> execute(HttpAsyncRequestProducer httpAsyncRequestProducer,
			final HttpAsyncResponseConsumer<T> httpAsyncResponseConsumer, final HttpContext httpContext,
		...
		try {
	        ...
	        BasicFuture<T> future = new BasicFuture<T>(futureCallback);
	        future.failed(e);
	        return future;
		...
		final Future<HttpResponse> resp = execute(httpAsyncRequestProducer.getTarget(), request, httpContext, new FutureCallback<HttpResponse>() {
			@Override
			...
			public void completed(HttpResponse httpResponse) {
				...
				httpAsyncResponseConsumer.responseCompleted(httpContext);
				futureCallback.completed(httpAsyncResponseConsumer.getResult());
			}
			@Override
		...
		final Future<T> future = new Future<T>() {
			...
			public T get() throws InterruptedException, ExecutionException {
				HttpResponse httpResponse = resp.get();
				try {
					httpAsyncResponseConsumer.responseReceived(httpResponse);
				} catch (IOException e) {
					CachingHttpAsyncClient.this.log.warn(e.getMessage(), e);
					return null;
				} catch (HttpException e) {
					CachingHttpAsyncClient.this.log.warn(e.getMessage(), e);
					return null;
				...
				httpAsyncResponseConsumer.responseCompleted(httpContext);
				return httpAsyncResponseConsumer.getResult();
			}
			@Override
			public T get(long timeout, TimeUnit unit)
					throws InterruptedException, ExecutionException,
					TimeoutException {
				HttpResponse httpResponse = resp.get(timeout, unit);
				try {
					httpAsyncResponseConsumer.responseReceived(httpResponse);
				} catch (IOException e) {
					CachingHttpAsyncClient.this.log.warn(e.getMessage(), e);
	...
	public Future<HttpResponse> execute(HttpUriRequest request, FutureCallback<HttpResponse> futureCallback) {
        ...
        return execute(request, context, futureCallback);
    }

    @Override
	public Future<HttpResponse> execute(HttpUriRequest request, HttpContext context, FutureCallback<HttpResponse> futureCallback) {
        URI uri = request.getURI();
        HttpHost httpHost = new HttpHost(uri.getHost(), uri.getPort(), uri.getScheme());
        return execute(httpHost, request, context, futureCallback);
    }
    ...
    @Override
	public Future<HttpResponse> execute(HttpHost target, HttpRequest request, HttpContext context, FutureCallback<HttpResponse> futureCallback) {
