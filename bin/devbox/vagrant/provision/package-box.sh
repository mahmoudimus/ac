...
vagrant destroy -f | tee target/logs/vagrant-destroy.logs
echo "Done."
echo "Running provisioning scripts..."
vagrant up | tee target/logs/vagrant-provision.logs
...
aws s3 cp target/atlassian-connect.box s3://atlassian-connect-devbox/bin/devbox/vagrant/provision/target/atlassian-connect.box
