...
def old_version_for_product(product_dict, properties_xml):
    product_version_xml = properties_xml.find(
        maven_pom_prepend + "atlassian." + product_dict["name"] + ".version")
...
def version_tuple_to_str(version):
    return str(version.major) + "." + str(version.minor) + "." + str(
        version.patch) + "-" + version.additional


def create_version_tuple(version_str):
    match = re.match(r'(\d+)\.(\d+)\.(\d+)(\-(.*))?', version_str)
    return VersionTuple(major=int(match.group(1)), minor=int(match.group(2)),
                        patch=int(match.group(3)), additional=match.group(5))
...
def main():
    ...
    if root.findtext(
                    maven_pom_prepend + "artifactId") != "atlassian-connect-parent":
        print WARN_PATH
        exit(2)
