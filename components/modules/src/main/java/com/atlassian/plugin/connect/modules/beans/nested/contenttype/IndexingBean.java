 ...
 * <p>
 * The <code>contentPropertyBody</code> property allow add-on to hook into the mechanism by which Confluence populates its
 * search index. Each time an Extensible Content Type is created or updated in Confluence, the content that stored in
 * the content property defined in the <code>contentPropertyBody</code> will be added to the search index.
 * <p>
 * This is useful when the body of the Extensible Content Type is not searchable, for example: JSON or binary data.
 * The add-on can still provide meaningful search text for this content via storing the extracted information to the
 * content property defined in <code>contentPropertyBody</code>.
 * <p>
 * Please consult <a href="https://developer.atlassian.com/confdev/confluence-rest-api/content-properties-in-the-rest-api">Content Properties in the REST API</a>
 * for how to store, modify and delete content property via REST API.
 * @since 1.1.77
 */
