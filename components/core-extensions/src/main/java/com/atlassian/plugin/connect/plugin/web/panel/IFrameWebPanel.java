...
public class IFrameWebPanel implements WebPanel
{
    ...
    public void writeHtml(final Writer writer, final Map<String, Object> context) throws IOException
    {
        if (condition.shouldDisplay(context))
        {
            ...
            Map<String, Object> contextParameters = new HashMap<>(pluggableParametersExtractor.extractParameters(context));
            IFrameContext iframeContext = substituteContext(contextParameters);
            writer.write(iFrameRenderer.render(iframeContext, "", Collections.EMPTY_MAP, remoteUsername, contextParameters));
        }
        else
        {
