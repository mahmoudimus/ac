...
import com.atlassian.jwt.JwtConstants.HttpRequests;
...
import com.atlassian.plugin.connect.core.HttpHeaderNames;
import com.atlassian.plugin.connect.core.applinks.ConnectApplinkManager;
import com.atlassian.plugin.connect.core.capabilities.BeanToModuleRegistrar;
...
import com.atlassian.plugin.connect.core.usermanagement.ConnectAddOnUserService;
...
public class ConnectAddonManager
{
    ...
    private Response getSyncHandlerResponse(String addOnKey, URI callbackUri, String jsonEventData, Option<String> authHeader) throws LifecycleCallbackException
    {
        try
        {
            ...
            {
                request.setHeader(HttpRequests.AUTHORIZATION_HEADER, authHeader.get());
            }
    ...
    {
        final Consumer consumer = Preconditions.checkNotNull(consumerService.getConsumer()); // checkNotNull() otherwise we NPE below
