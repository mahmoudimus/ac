...
public class AddOnPropertyServiceImpl implements AddOnPropertyService
{
            ...
            @Nonnull final Function<Option<AddOnProperty>, ServiceConditionResult<T>> testFunction)
    {
        ...
        if (validationResult.isValid())
        {
            ...
            return store.executeInTransaction(new AddOnPropertyStore.TransactionAction<PutServiceResult<T>>()
            {
                @Override
                public PutServiceResult<T> call()
                {
                    ...
                    {
                        final AddOnPropertyStore.PutResultWithOptionalProperty putResult = store.setPropertyValue(input.getAddOnKey(), input.getPropertyKey(), input.getValue());
            ...
            @Nonnull final Function<Option<AddOnProperty>, ServiceConditionResult<T>> testFunction)
    {
        ...
        if (validationResult.isValid())
        {
            ...
            return store.executeInTransaction(new AddOnPropertyStore.TransactionAction<DeleteServiceResult<T>>()
            {
                @Override
    ...
    {
        private static final Map<AddOnPropertyStore.PutResult, OperationStatus> mapping =
                ImmutableMap.<AddOnPropertyStore.PutResult, OperationStatus>builder()
                        .put(AddOnPropertyStore.PutResult.PROPERTY_CREATED, OperationStatusImpl.PROPERTY_CREATED)
                        .put(AddOnPropertyStore.PutResult.PROPERTY_LIMIT_EXCEEDED, OperationStatusImpl.MAXIMUM_PROPERTIES_EXCEEDED)
                        .put(AddOnPropertyStore.PutResult.PROPERTY_UPDATED, OperationStatusImpl.PROPERTY_UPDATED)
                        .build();

        public static OperationStatus mapToServiceResult(AddOnPropertyStore.PutResult putResult)
        {
