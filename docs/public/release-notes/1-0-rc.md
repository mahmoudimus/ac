...
* Breaking changes to avoid signing of unverified content (see also the introduction in [Authentication](../concepts/authentication.html) on why this became necessary):
   * TODO: add macro parameter change description here
   * The JavaScript method `dialog.getIframeHtmlForUrl(...)` is no longer supported. Consider other dialog options or use `getIframeHtmlForKey(...)` from `host/content` instead.
