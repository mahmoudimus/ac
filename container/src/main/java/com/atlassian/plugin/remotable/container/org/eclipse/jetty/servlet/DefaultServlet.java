...
/**
 * This is a copy of {@link org.eclipse.jetty.servlet.DefaultServlet} with slight changes to work around the issue
 * reported as:
 * <ul>
 *     <li><a href="https://remoteapps.jira.com/browse/QA-13">QA-13</a></li>
 *     <li>but also as <a href="https://issues.apache.org/jira/browse/FELIX-1032">FELIX-1032</a></li>
 * </ul>
 *
 * Note that this doesn't fix the underlying issue which we believe is a JDK issue, but rather works around it.
 */

package com.atlassian.plugin.remotable.container.org.eclipse.jetty.servlet;
 ...
 *
 */
public final class DefaultServlet extends HttpServlet implements ResourceFactory
{
    public static final int QA_13_WORK_AROUND_ATTEMPTS = 10;
    ...
    {
        return getResource(pathInContext, 0);
    }

    private Resource getResource(String pathInContext, int attempt)
    {
        if (attempt < QA_13_WORK_AROUND_ATTEMPTS)
        {
            try
            {
                final Resource resource = doGetResource(pathInContext);
                if (LOG.isDebugEnabled())
                    LOG.debug("Got the resource '" + pathInContext + "' after " + (attempt + 1) + " attempts!");
                return resource;
            }
            catch (NullPointerException e)
            {
                if (isQA13Exception(e))
                {
                    // try once more
                    return getResource(pathInContext, attempt + 1);
                }
                else
                {
                    // simply rethrow, this is another error!
                    throw e;
                }
            }
        }
        ...
        {
            throw new IllegalStateException("Could not get the resource '" + pathInContext + "'after " + QA_13_WORK_AROUND_ATTEMPTS + " attempts");
        }
    }

    private boolean isQA13Exception(NullPointerException e)
    {
        return e.getMessage().equals("Inflater has been closed");
    }

    /**
     * This is the original code that implemented the {@link DefaultServlet#getResource(String)}.
     */
    private Resource doGetResource(String pathInContext)
    {
        Resource r=null;
        if (_relativeResourceBase!=null)
